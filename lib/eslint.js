...
module.exports = (function() {
     ...
     * @param {number} severity 0, 1, or 2, indicating rule severity
     * @param {Object} meta Metadata of the rule
     * @returns {boolean} true if can fix.
     */
    function fixable(fix, severity, meta) {
        if (!fix) {
            return false;
        }
        if (!severity) {
            ...
            Object.keys(config.rules).filter(key => getRuleSeverity(config.rules[key]) > 0).forEach(key => {
                ...
                try {
                    const ruleContext = new RuleContext(
                        ...
                        fixable(fix, severity, ruleCreator.meta)
                    );
